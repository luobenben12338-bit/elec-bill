...
- **深浅主题**：顶部一键切换深色和浅色显示，选择会保存在当前设备，移动端系统栏会同步配色并适配安全区
...
- **多设备同步**：按记录 ID 和修改时间合并，删除状态也会同步；冲突时自动拉取最新版本重新合并后推送
...
- 数据格式 v4：记录带修改时间，删除使用墓碑，旧版数据会自动迁移
- 云端同步使用防抖推送（1.2 秒延迟）避免频繁请求，冲突时按最新修改重新合并后重试
