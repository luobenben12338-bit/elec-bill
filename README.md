...
- **多页面架构**：首页 / 统计 / 记录 / 我的 四个视图，底部凸起按钮一键呼出录入弹层
...
- **移动端与桌面端适配**：手机端底部导航 + 底部抽屉录入，桌面端（≥768px）顶部导航 + 居中弹窗 + 内容居中限宽
- **深浅主题**：顶部一键切换深色和浅色显示，选择会保存在当前设备，移动端系统栏会同步配色并适配安全区
- **中英双语**：界面支持中文 / English 切换，自动跟随系统语言，选择保存在本地，日期与数字按语言习惯格式化
...
- 响应式适配通过 CSS Media Queries（断点 768px）和 touch-action / pointer 事件实现，左滑删除、下拉刷新、底部抽屉下拉关闭均支持触摸手势
- hash 路由：视图切换通过 location.hash 深链直达，白名单兜底回首页，history.replaceState 避免污染返回历史
- i18n：内置中英双语字典，data-i18n 标记批量切换，三级回退（当前语言 → 中文 → key），启动时键对齐自检
- 响应式断点 768px：桌面端切换为顶部导航 + 居中弹窗 + 内容居中限宽
- 下拉刷新：overscroll-behavior 约束 + 顶部指示器，仅在 scrollTop=0 时触发，避免与 iOS 回弹手势冲突
...
部署后用手机浏览器打开应用，在「我的」页面点击「📱 安装应用」按钮即可添加到手机主屏幕。安装后以全屏模式运行，体验接近原生应用，离线也能查看历史数据和本地记账。
